...
In other words, **`state()` is powerful, but overkill for single values**.
You don't need a full object structure when you just want to track one thing.
...
```
ref(0)  →  state({ value: 0 })  →  Returns reactive container

You interact with:     count.value
Instead of:           counter.count
```
...
effect(() => {
  document.getElementById(
    "timer"
  ).textContent = `Time remaining: ${seconds.value}s`;
});
...
### Multiple Refs Working Together
...
effect(() => {
  ...
  document.getElementById(
    "perimeter"
  ).textContent = `Perimeter: ${perimeter}px`;
});
...
effect(() => {
  // One large effect doing multiple things
...
- ⚠️ For complex objects with many properties, use `state()` instead
- ⚠️ Mutating object values won't trigger updates - replace the entire value
- ⚠️ Don't destructure - it breaks reactivity
...
➡️ Next, you might want to learn about [`state()`](state.md) for complex reactive objects or [`effect()`](effect.md) to make your refs truly reactive.
